...
def payment_process(request):
    return render(request,"network/payment_process.html")


def tripcoor(request):
    return render(request,"network/tripcoor.html")
...
def payment(request, username):
    if request.method == "POST":
        try:
            ...
            a = Subscription(user=user, type=type, amount=amount)
            a.save()
            return render(request, 'network/payment.html')
        except:
            messages.info(request, 'You Already Subscribed')
            return redirect('subscription')


def edit_profile(request, id):
    myuser = User.objects.get(id=id)
    template = loader.get_template('network/edit_profile.html')
